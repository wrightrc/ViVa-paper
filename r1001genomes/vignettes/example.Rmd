...
## Plotting Nei's Diversity Statistic
In addition to calculating the nucleotide diversity of a sequence like the transcipt of a gene, we can also plot the diversity statistic by individual position along the reference genome. This could be usefull if you suspect a particular region of your gene may be more important to function and or more sensitive to nucelotide or amino acid changes. 

<!--=========================================================================-->
<div  class="using-app-insert"><!--panel start--------------------------------->
## Plotting Nucleotide Diversity in the App
The second tab of the app, labeld "Diversity Plot" allows you to plot the nucleotide diversity for your favorite genes you selected in the "SNP Stats" tab.
```{r, echo = FALSE, out.width = "90%", out.extra='style="margin: auto; display: block; padding-top: 15px;"' }
imagePath <- system.file("extdata", "Screenshots", "Tab_2_Select_Gene.PNG", package = "r1001genomes")
...
```{r, echo = FALSE, out.width = "90%", out.extra='style="margin: auto; display: block; padding-top: 15px;"' }
imagePath <- system.file("extdata", "Screenshots", "Tab_2_Plot_Selection_Box.PNG", package = "r1001genomes")
knitr::include_graphics(imagePath)
knitr::asis_output("
                   ") # This newline required for headings to render after image
```
You should see a plot like the one above. The plot shows Nei's nucleotide diversity by <strong>nucleotide position</strong> of the protein coding region of the gene, though the X axis is marked by codon number from the N-terminus for convenience. The symbols are differentiated by whether the site has a synonymous or missense variant. If multiple variant types are present at theh same loci of the reference genome, both markers will be plotted.

Below The plot is a grey box. If you click and drag a selection box accross some points in the plot, some additional details on those points will appear in the grey box.

Below the plot is a data table with this information provided for all points on the plot. You can click the download button above the table to get a .csv file of this complete table.

</div><!--panel end------------------------------------------------------------>
<div  class="using-R-insert"><!--panel start----------------------------------->

## Using R
The app uses the function `get_coding_diversity` to calculate the diversity stat at each nucleotide position of the coding sequence. then uses the function `plot_coding_diversity` to plot that information for a single gene. we can do the same thing for all our genes in the following manner:
```{r out.width = "90%", fig.height=4,fig.width=7, out.extra='style="margin: auto; display: block; padding-top: 15px;"' }
