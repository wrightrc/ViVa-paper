...
A_ARFs <- alignCDS(A_IAAs$transcript_ID)
# make an alignment data frame
...
A_IAAs$tair_symbol %<>% fct_reorder(
    {as.integer(str_remove(A_IAAs$tair_symbol, 
                           pattern = "ARF"))}, 
    .desc = TRUE)
A_IAAs <- addSNPsToAlnDF(A_IAAs, VCF)

p <-ggplot(A_IAAs, aes(x = aln_pos, y = tair_symbol,
                             group = seq_pos, text = variants)) +
  geom_raster(data = A_IAAs[A_IAAs$seq_pos == "-",], fill = "lightgrey") +
  geom_raster(data = na.omit(A_IAAs), 
             # mapping = aes(fill = effects)) +
              mapping = aes(fill = as.integer(as.factor(effects)))) +
  #scale_fill_viridis(option = "A", discrete = TRUE) +
   scale_fill_viridis(breaks = range(
     as.integer(as.factor(na.omit(A_IAAs$effects)))),
                      labels = c("deleterious", "neutral"),
                      option = "A", 
