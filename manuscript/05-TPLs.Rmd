...
(ref:TPLsAlignment) **Alignment of A. thaliana TPL/TPR genes showing variants.** This alignment should probably be paired down to a key region rather than the eniter AA sequence.

```{r TPLsAlignment, fig.height = 20, fig.cap = "(ref:TPLsAlignment)"}
...
aln_df$tair_symbol <- fct_rev(aln_df$tair_symbol)
aln_df <- chunkAlnDF(aln_df = aln_df, chunk_width = 55)
VCF <- ldply(.data = VCF, .fun = subset, gt_GT != "0|0")
VCF <- getCodingDiv(VCF)
aln_df <- addSNPsToAlnDF(aln_df, VCF)

# aln_df_chunk <- chunkAlnDF(aln_df = aln_df, chunk_width = 55)
p <-ggplot(aln_df_chunk, aes(x = aln_pos, y = tair_symbol,
                             group = seq_pos, text = variants))
p <- p +
  geom_tile(data = na.omit(aln_df_chunk), mapping = aes(fill = effects),
  ...
  scale_x_continuous(breaks=seq(1,max(aln_df_chunk$aln_pos), by = 10)) +
  scale_y_discrete() +
